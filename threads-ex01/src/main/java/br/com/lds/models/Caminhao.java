...
public class Caminhao implements Runnable {
	...
	public void run() {
		...
		System.out.println("Caminhao " + this.nome + " terminou de ser carregados");
	}

	private int converteSegundosEmMili(int segundos) {
	...
	private void carregarCaminhao(int numeroDaCaixa) {
		...
		for (int i = 0; i < capacidadeDaCaixa; i++) {
			...
			try {
				...
				Thread.sleep(converteSegundosEmMili(this.intervalo));
			...
			System.out.println(
					"O pao " + i + " foi inserido na caixa numero " + numeroDaCaixa + " o no caminhao " + this.nome);
		}
